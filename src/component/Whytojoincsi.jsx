...
function WhyToJoin() {
  return (
    <div className='relative mx-auto px-4 sm:px-6 md:py-[4vw] md:px-[7vw] my-[10vw]'>
      <div className=''>
        <img 
          src="../../public/assets/tojoin_bg.svg" 
          alt="Background" 
        ...
        <div className='flex flex-col px-4 py-6 md:px-12 md:py-8 gap-6 md:gap-10 w-full md:w-[60vw]'>
          {sections.map(({ id, title, description }) => (
            <div key={id} className='flex flex-col items-start gap-2'>
              <div className='text-2xl md:text-3xl text-[#cccbcb]'>
                {id}
              </div>
              <hr className="border-t-2 border-[#658fce] w-[80%]"/>
              <div className='text-[#1c335d] text-xl md:text-2xl'> 
                {title}
              </div>
              <div className='text-[#727272] text-start text-base md:text-xl font-productsans font-light'>
                {description}
              </div>
            </div>
          ))}
